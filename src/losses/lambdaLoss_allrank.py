"""LambdaLoss family of LTR losses, vendored from allRank.

Source: https://github.com/allegro/allRank/blob/master/allrank/models/losses/lambdaLoss.py
License: Apache License 2.0 (https://www.apache.org/licenses/LICENSE-2.0)

Reference: Wang, Li, Najork et al., "The LambdaLoss Framework for Ranking
Metric Optimization", CIKM 2018.

The original module imports `PADDED_Y_VALUE` and `DEFAULT_EPS` from allRank's
internal modules. We inline both as constants so this file is self-contained
and depends only on torch.

Compared to our previous in-house `lambdarank.py`:
  * supports padded slates via `padded_value_indicator` (default -1)
  * supports multiple weighting schemes (lambdaRank_scheme,
    ndcgLoss1_scheme, ndcgLoss2_scheme, ndcgLoss2PP_scheme,
    rankNet_scheme, rankNetWeightedByGTDiff_scheme,
    rankNetWeightedByGTDiffPowed_scheme)
  * input shape is [batch_size, slate_length] — for our flat per-batch
    ranking, pass [1, B]; for grouped (e.g. per-TA) ranking, pass
    [n_groups, max_slate_length] with -1 padding.
"""
