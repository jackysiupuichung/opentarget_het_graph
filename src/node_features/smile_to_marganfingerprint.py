...
def build_drug_embeddings(args: argparse.Namespace) -> Dict[str, np.ndarray]:
    ...
    # Optional PCA (2D, visualization only)
    pca_coords = None
    if args.run_pca and embeddings:
    ...
    return embeddings, skipped_biologics, pca_coords
...
def main():
    args = parse_args()
    embeddings, skipped, pca_coords = build_drug_embeddings(args)
    save_outputs(embeddings, skipped, pca_coords, args)
