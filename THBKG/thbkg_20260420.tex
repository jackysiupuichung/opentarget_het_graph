...
availability at the clinical decision point, for all seven experimental
conditions of Table~\ref{tab:conditions} plus RDG and OTS. Strata:
\textit{evidence\_free} (no direct target--disease evidence),
\textit{literature\_only} (text-mined co-occurrence only),
...
references; the full RR@$N$ curves for all conditions appear in
Figure~\ref{fig:rr_by_limit_ablation}, and per-TA and per-stratum
...
Targets 23.06; single training run per condition.}
\label{tab:ablation}%
}{%
...
\caption{Relative risk by top-$N$ cutoff for all seven experimental
conditions plus RDG and OTS, averaged across 13 therapeutic areas under
...
\label{fig:rr_by_limit_ablation}
\end{figure}
...
B3) by $\approx 1.5\times$ at RR@100.

\noindent\textit{Caveat.} The ablation numbers are single-seed. The
validation early-stopping signal is sharp around the test peak (see
Methods), and multi-seed runs are needed before the precise margins are
quoted as definitive; the qualitative ordering of conditions ---
edge-feature modulation $\gg$ encoder choice, score $>$ novelty,
both $>$ either --- is robust to which equivalent-near-peak epoch is
selected.
...
\textbf{Ablation analysis.} Section~\ref{sec:ablation} isolates the
sources of the \eahgt{} advantage across the seven conditions of
Table~\ref{tab:conditions}. The dominant factor is the edge-feature
modulation, not the encoder family: plain \hgt{} (B1) is the weakest
condition, and adding the cumulative score and novelty score to the
\eahgt{} attention (P1 $\to$ P3) accounts for almost the entire lift.
Among the edge-feature-free baselines, \rgcn{}'s explicit per-relation
weight matrices give it the strongest top-$N$ enrichment, ahead of
\gatv{} and \compgcn{}; but every \eahgt{} variant that uses the score
feature beats all of them, confirming that heterogeneity is necessary
but not sufficient. The single-seed nature of these runs is the main
limitation; the ordering of conditions is robust but multi-seed
estimates are needed before the precise margins are treated as final.
...
stratified by both evidence stratum (pioneer/known, evidence density)
and therapeutic area, for all seven experimental conditions of
Table~\ref{tab:conditions} plus RDG and OTS. The P3 (\eahgt{})
advantage on AP is most pronounced in the \textit{evidence\_free} and
\textit{literature\_only} strata for pioneer targets.}
