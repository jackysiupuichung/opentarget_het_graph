...
evidential score. $n_{ij}$: Open Targets novelty score. ``Edge feats''
denotes whether the continuous per-edge attributes enter message passing.}
\label{tab:conditions}%
}{%
\begin{tabular*}{\columnwidth}{@{}lll@{\extracolsep{\fill}}p{4.2cm}@{}}
\toprule
\textbf{ID} & \textbf{Model} & \textbf{Edge feats} &
\textbf{Primary claim tested} \\
\colrule
B1 & \hgt{}     & None               & Type-conditioned attention only \\
B2 & \gatv{}    & None               & Dynamic attention, external heterogeneity \\
B3 & \rgcn{}    & None               & Per-relation weight matrices \\
B4 & \compgcn{} & None               & Composition-based, parameter-efficient relations \\
\colrule
P1 & \eahgt{}   & $s_{ij}$ only      & Score modulation with full heterogeneity \\
P2 & \eahgt{}   & $n_{ij}$ only      & Novelty modulation with full heterogeneity \\
P3 & \eahgt{}   & $s_{ij},\, n_{ij}$ & Both, the proposed model \\
\botrule
\end{tabular*}%
...
their concatenation $[\mathbf{h}_t \concat \mathbf{h}_d] \in
\mathbb{R}^{2d}$ to a scalar ranking score:
\begin{align}
    \mathbf{z}_1 &= \operatorname{ReLU}(\mathbf{W}_1\,[\mathbf{h}_t \concat \mathbf{h}_d] + \mathbf{b}_1), &\quad &\mathbf{W}_1 \in \mathbb{R}^{d \times 2d}, \nonumber\\
    \mathbf{z}_2 &= \operatorname{ReLU}(\mathbf{W}_2\,\mathbf{z}_1 + \mathbf{b}_2), &\quad &\mathbf{W}_2 \in \mathbb{R}^{(d/2) \times d}, \nonumber\\
    \hat{y} &= \mathbf{w}_3^\top\,\mathbf{z}_2 + b_3, &\quad &\mathbf{w}_3 \in \mathbb{R}^{d/2}.
    \label{eq:decoder}
\end{align}
...
averages across the 13 retained therapeutic areas; per-therapeutic-area
RR@$N$ point estimates carry 95\% Katz log-method confidence intervals
\citep{Katz1978ObtainingStudies}.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/results/relative_risk_by_limit_ablation.png}
...
mark the top 1\% and top 2\% of all test pairs. The proposed P3
(\eahgt{} with both edge features) is the top model at every cutoff;
all \hgt{}-with-edge-features variants and \rgcn{} exceed RDG across
the actionable range.}
